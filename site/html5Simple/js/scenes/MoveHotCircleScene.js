/**
 * MoveHotCircleScene
 * Hot circle with radius and 3D tilts. Tap eases the center to the click.
 */
class MoveHotCircleScene extends Phaser.Scene {
    constructor() {
        super({ key: 'MoveHotCircleScene' });
        console.log('[MoveHotCircleScene] Constructor called');
    }
    ...
    create() {
        enterTestsCanvas(this);
        console.log('[MoveHotCircleScene] create() started');
        fxEnter(this);
        const width = this.cameras.main.width;
        const height = this.cameras.main.height;

        this.add.text(width / 2, 56, 'Move hot circle', {
            font: `${UI.heading}px monospace`,
            fill: '#000000'
        }).setOrigin(0.5);

        this.add.text(width / 2, 88, 'Tap to move the circle', {
    ...
    createTestPanel(x, y, width, height) {
        ...
        const inset = 2;
        this.heatMap = attachHotCircle(this, x + inset, y + inset, width - inset * 2, height - inset * 2);
    ...
    createSettingsPanel(x, y, width, height) {
        ...
        const tabGap = 6;
        const tabY = y + 18;
        const tabsRight = x + width - 12;
        this.heatTabBtn = this.createTabButton(tabsRight - tabW * 2.5 - tabGap * 2, tabY, tabW, tabH, 'HEAT', () => {
            this.setSettingsTab('heat');
        });
        this.geoTabBtn = this.createTabButton(tabsRight - tabW * 1.5 - tabGap, tabY, tabW, tabH, 'GEO', () => {
            this.setSettingsTab('geo');
        });
        this.windTabBtn = this.createTabButton(tabsRight - tabW / 2, tabY, tabW, tabH, 'WIND', () => {
            this.setSettingsTab('wind');
        });

        const heatRows = [
            { key: 'diffusion', label: 'DIFFUSE', decimals: 1 },
            { key: 'cooling', label: 'COOL', decimals: 2 },
            { key: 'energy', label: 'CIRCLE HEAT', decimals: 1 },
            { key: 'noiseAmt', label: 'NOISE', decimals: 1 },
            { key: 'noiseGrain', label: 'GRAIN', decimals: 1 },
            { key: 'noiseFlicker', label: 'FLICKER', decimals: 1 }
        ];
        const geoRows = [
            { key: 'radius', label: 'RADIUS', decimals: 0 },
        ...
        const windRows = [
            { key: 'windForce', label: 'FORCE', decimals: 0 },
            { key: 'windDir', label: 'DIR', decimals: 0 },
            { key: 'windGust', label: 'GUST', decimals: 1 },
            { key: 'windSpeed', label: 'X SPEED', decimals: 0 }
        ];
        const rowH = 32;
        const rowTop = y + 38;
        this.settingsValues = {};
        ...
        this.windTabBody = this.add.container(0, 0);
        ...
        windRows.forEach((row, index) => {
            this.createStepperRow(this.windTabBody, x + 8, rowTop + index * rowH, width - 16, rowH, row);
        });
    ...
    setSettingsTab(tab) {
        ...
        const tabs = [
            { id: 'heat', body: this.heatTabBody, btn: this.heatTabBtn },
            { id: 'geo', body: this.geoTabBody, btn: this.geoTabBtn },
            { id: 'wind', body: this.windTabBody, btn: this.windTabBtn }
        ];
        tabs.forEach((slot) => {
            const on = slot.id === tab;
            slot.body.setVisible(on);
            this.setTreeInputEnabled(slot.body, on);
            this.redrawTabButton(slot.btn, on);
        });
    }
    ...
    onBackClick() {
        console.log('[MoveHotCircleScene] Back clicked');
