/**
 * MoveHotRodScene
 * Horizontal hot line that eases to each tap (Y of the click).
 */
class MoveHotRodScene extends Phaser.Scene {
    constructor() {
        super({ key: 'MoveHotRodScene' });
        console.log('[MoveHotRodScene] Constructor called');
    }
    ...
    create() {
        enterTestsCanvas(this);
        console.log('[MoveHotRodScene] create() started');
        fxEnter(this);
        const width = this.cameras.main.width;
        const height = this.cameras.main.height;

        this.add.text(width / 2, 56, 'Move hot rod', {
            font: `${UI.heading}px monospace`,
            fill: '#000000'
        }).setOrigin(0.5);

        this.add.text(width / 2, 88, 'Tap to move the line', {
    ...
    createTestPanel(x, y, width, height) {
        ...
        const inset = 2;
        this.heatMap = attachHotRod(this, x + inset, y + inset, width - inset * 2, height - inset * 2);
    ...
    onBackClick() {
        console.log('[MoveHotRodScene] Back clicked');
