/**
 * ShardScene
 * Copy of BackHighlightScene. Skew 0 is a rectangle; other skew is a parallelogram.
 * GLISTEN sweeps the heat from the shape's left edge to its right edge.
 */
class ShardScene extends Phaser.Scene {
    constructor() {
        super({ key: 'ShardScene' });
        console.log('[ShardScene] Constructor called');
    }
    ...
    create() {
        enterTestsCanvas(this);
        console.log('[ShardScene] create() started');
        fxEnter(this);
        const width = this.cameras.main.width;
        const height = this.cameras.main.height;

        this.add.text(width / 2, 56, 'Shard', {
            font: `${UI.heading}px monospace`,
            fill: '#000000'
        }).setOrigin(0.5);

        this.add.text(width / 2, 88, 'Glisten sweeps left to right', {
    ...
    createTestPanel(x, y, width, height) {
        ...
        const inset = 2;
        this.heatMap = attachBackHighlight(this, x + inset, y + inset, width - inset * 2, height - inset * 2);
        this.heatMap.from.skew = 0;
        this.heatMap.to.skew = 40;
        this.heatMap.current.skew = 0;
        this.heatMap.from.glisten = 0;
        this.heatMap.to.glisten = 100;
        this.heatMap.current.glisten = 0;
        this.heatMap.params.cooling = 1.6;
    ...
    createSettingsPanel(x, y, width, height) {
        ...
        const poseRows = [
            ...
            { key: 'glisten', label: 'GLISTEN', decimals: 0 }
        ];
        const fieldRows = [
    ...
    onBackClick() {
        console.log('[ShardScene] Back clicked');
