/**
 * BackHighlightScene
 * Filled hot rectangle. CYCLING eases FROM → TO → FROM, with DELAY and GAP pauses.
 */
class BackHighlightScene extends Phaser.Scene {
    constructor() {
        super({ key: 'BackHighlightScene' });
        console.log('[BackHighlightScene] Constructor called');
    }
    ...
    create() {
        enterTestsCanvas(this);
        console.log('[BackHighlightScene] create() started');
        fxEnter(this);
        const width = this.cameras.main.width;
        const height = this.cameras.main.height;

        this.add.text(width / 2, 56, 'Back highlight', {
            font: `${UI.heading}px monospace`,
            fill: '#000000'
        }).setOrigin(0.5);

        this.add.text(width / 2, 88, 'Filled rectangle, tap to move', {
    ...
    onBackClick() {
        console.log('[BackHighlightScene] Back clicked');
