...
flowchart TD
  ...
  Tests -->|COMPOSITE UI EDITOR| CompositeEditor[CompositeUIEditorScene]
  ...
  Tests -->|BACK HIGHLIGHT| BackHighlight[BackHighlightScene]
  Tests -->|SHARD| Shard[ShardScene]
  Tests -->|SHEET| Sheet[SheetScene]
  Tests -->|MOVE HOT CIRCLE| HotCircle[MoveHotCircleScene]
  Editor -->|BACK| Tests
  CompositeEditor -->|BACK| Tests
  UITest -->|BACK| Tests
  HotMap -->|BACK| Tests
  RotatedHotMap -->|BACK| Tests
  HotRod -->|BACK| Tests
  BackHighlight -->|BACK| Tests
  Shard -->|BACK| Tests
  Sheet -->|BACK| Tests
...
| MenuScene | [`js/scenes/MenuScene.js`](js/scenes/MenuScene.js) | Full screen | Title **Sweet Adventure**. Subtitle **Pick a match**. Player name in the corner. Buttons: LOGIN, REGISTER, LOGOUT, INVENTORY (stub), START GAME, UI TESTS, EXIT (stub). Bottom **SKIN IN/OFF**: **IN** fills the menu background with the heat-map preset ([`resources/ui-editor-init.json`](resources/ui-editor-init.json)), including a highlight on each of the seven menu buttons, and cuts the button labels out of a black fill so the heat map shows through the letters; **OFF** keeps the blank background and outline buttons. |
...
| TestsScene | [`js/scenes/TestsScene.js`](js/scenes/TestsScene.js) | Full screen, **780×844** (2× game width) | Title **Tests**. Subtitle **Pick a sandbox**. Buttons in two columns: left UI EDITOR, COMPOSITE UI EDITOR, UI PLAZMA BALL, HOT MAP BALL, ROTATED HOT MAP BALL, MOVE HOT ROD; right BACK HIGHLIGHT, SHARD, SHEET, MOVE HOT CIRCLE, BACK. Entering this hub widens the Phaser canvas; child sandboxes use that width. BACK to Menu restores **390×844**. |
| CompositeUIEditorScene | [`js/scenes/CompositeUIEditorScene.js`](js/scenes/CompositeUIEditorScene.js) | Stage **390×844**, tools canvas beside it | Copy of UIEditorScene. Same stage and tools layout. Tools canvas is [`CompositeUIEditorToolsScene`](js/scenes/CompositeUIEditorToolsScene.js). BACK returns to Tests. |
| UIEditorScene | [`js/scenes/UIEditorScene.js`](js/scenes/UIEditorScene.js) | Stage **390×844**, tools canvas beside it | Two canvases. The stage canvas is the main game frame, same size and scale as the main screen. **STAGE** hosts one [`UiHeatMap`](js/ui/UiHeatMap.js) with three [`UiHotBall`](js/ui/UiHotBall.js), three [`UiHotRod`](js/ui/UiHotRod.js), and two [`UiBackHighlighter`](js/ui/UiBackHighlighter.js) emitters. Green frames ([`UiStageMarkup`](js/ui/UiStageMarkup.js)) sit on top of that field in the same places as the main-scene panels and buttons for the active layout, including the extended Effects row. They are transparent, non-interactive markup; taps pass through to the heat field. **EFFECTS** **HEAT** **FRAMES** (ON/OFF) shows or hides them. **MENU** (ON/OFF, starts OFF) shows the same kind of frames for the menu: LOGIN, REGISTER, LOGOUT, INVENTORY, START GAME, UI TESTS, EXIT, SKIN, LAYOUT, and FX. The second canvas ([`UIEditorToolsScene`](js/scenes/UIEditorToolsScene.js)) holds **EFFECTS** (FRAMES, MENU, DIFFUSE, COOL, VIEW, PLAY / STEP / STOP) above **PROPS**, with **OBJECTS** on its right. Title **UI Editor** sits on that canvas. The page scrolls sideways when both canvases do not fit. **OBJECTS** is a scrollable name-and-type list with **COPY** and **DEL** on each row. A click selects that object and updates **PROPS**. **COPY** duplicates the type and props, shifts rod **START X/Y** or ball/highlight **CENTER X/Y**, and adds the suffix **copy**. **SAVE** / **LOAD** a local JSON preset. **PROPS** edits that object: balls (VISIBLE, DOT SIZE/HEAT, GEO including PHASE and live CENTER X/Y), rods (VISIBLE, LINE HEAT, GEO, MOVE with START/END X% Y% from −200 to 200, START/END ANG, DELAY at end, GAP at start), or back highlights (FIELD VISIBLE, CYCLING, SPEED, DELAY, GAP, CENTER X% Y%; FROM/TO WIDTH, HEIGHT, ANGLE, HEAT). A highlight eases FROM → TO, pauses DELAY, eases back when CYCLING is on, pauses GAP, then repeats. Tap Stage to move the selected object. BACK on the tools canvas returns to Tests and restores the Tests width. |
...
| BackHighlightScene | [`js/scenes/BackHighlightScene.js`](js/scenes/BackHighlightScene.js) | Full screen | Title **Back highlight**. Subtitle **Filled rectangle, tap to move**. A filled hot rectangle stamps into the heat field ([`BackHighlightFx`](js/fx/BackHighlightFx.js)). **WIDTH**, **HEIGHT**, **ANGLE**, and **HEAT** ease from **FROM** to **TO**, pause **DELAY**, then either ease back (**CYCLING** on) or snap to **FROM** (**CYCLING** off). **GAP** pauses at **FROM** before the next pass. A tap eases the rectangle center to that point. BACK returns to Tests. |
| ShardScene | [`js/scenes/ShardScene.js`](js/scenes/ShardScene.js) | Full screen | Title **Shard**. Subtitle **Glisten sweeps left to right**. **SKEW** 0 is a rectangle; other values lean it into a parallelogram. **GLISTEN** is on **FROM** and **TO**: **0** is the left edge, **100** is the right, and it eases with the pose. **COOL** sets how fast the trail fades. BACK returns to Tests. |
| SheetScene | [`js/scenes/SheetScene.js`](js/scenes/SheetScene.js) | Full screen | Title **Sheet**. Subtitle **Cosine sheet. Glisten repeats**. The ribbon centerline is cos from 0 to 2π. **WIDTH** is one turn, **HEIGHT** the thickness, **BEND** the wave height, **PHASE** shifts the cosine (degrees, eased FROM → TO), and **SKEW** leans the cross-section. The wave stays horizontal. Heat eases **FROM** to **TO**. **GLISTEN** on **FIELD** turns a bright band on or off. When on, a hot band sweeps along the curve on **GL SPEED**, pauses **GL DELAY**, and repeats. **GL POWER** is how much heat that band stamps into the field. **BALL** is two hot dots (**DOT SIZE**, **DOT HEAT**). One runs the centerline start to end, **DELTA** cells below it. The other runs end to start, **DELTA** cells above it. Both repeat ([`SheetFx`](js/fx/SheetFx.js)). BACK returns to Tests. |
...
| Tests | Back highlight | Fade (BACK HIGHLIGHT). |
...
| Back highlight | Tests | Fade (BACK). |
