...
| SheetScene | [`js/scenes/SheetScene.js`](js/scenes/SheetScene.js) | Full screen | Title **Sheet**. Subtitle **Heat cycles. Glisten repeats**. The parallelogram heat eases **FROM** to **TO** like Back Highlight, with **SKEW**. **GLISTEN** is not a pose: a bright band sweeps left to right on **GL SPEED**, pauses **GL DELAY**, and repeats ([`SheetFx`](js/fx/SheetFx.js)). BACK returns to Tests. |
