...
describe('Round', () => {
  describe('constructor', () => {
    ...
    it('should create a round with subRounds', () => {
      ...
      const subRound1 = new SubRound(1, [move1], startTime, startTime + 1000, startTime);
      const subRound2 = new SubRound(2, [move2], startTime + 1000, startTime + 2000, startTime + 1000);
      const round = new Round('round-1', [subRound1, subRound2], RoundStatus.Current, startTime);
    ...
    it('should throw ValidationError for invalid subRounds array', () => {
      expect(() => new Round('round-1', 'not-an-array' as any, RoundStatus.Pending, Date.now())).toThrow(ValidationError);
    });
  ...
  describe('toJSON', () => {
    it('should convert round to JSON', () => {
      ...
      const startTime = Date.now();
      const subRound = new SubRound(1, [move], startTime, startTime + 1000, startTime);
      const round = new Round('round-1', [subRound], RoundStatus.Finished, startTime, 'winner-user-1', startTime + 1000);
